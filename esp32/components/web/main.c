...
static TaskHandle_t worker_handles[CONFIG_HTTP_MAX_ASYNC_REQUESTS];
...
static void start_workers(void)
{
    ...
    worker_ready_count = xSemaphoreCreateCounting(
        CONFIG_HTTP_MAX_ASYNC_REQUESTS,  // Max Count
        0); // Initial Count
    ...
    // start worker tasks
    for (int i = 0; i < CONFIG_HTTP_MAX_ASYNC_REQUESTS; i++) {

        bool success = xTaskCreatePinnedToCore(worker_task, "async_req_worker",
                                    CONFIG_HTTP_ASYNC_WORKER_TASK_STACK_SIZE, // stack size
                                    (void *)0, // argument
...
static esp_err_t index_handler(httpd_req_t *req)
{
    ...
    const char* html = "<div><a href=\"/long\">long</a></div>"
        ...
        "<div><a href=\"/myquick\">myquick</a></div>";
...
static httpd_handle_t start_webserver(void)
{
    ...
    config.max_open_sockets = CONFIG_HTTP_MAX_ASYNC_REQUESTS + 1;
    ...
    httpd_register_uri_handler(server, &quick_uri1);
